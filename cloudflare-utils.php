...
class PP_Cloudflare_Utils {
	...
	public function clear_cache_on_new_comment( $comment_id, $comment_object ) {
		if ( $comment_object->comment_approved === '1' ) {
			$this->clear_cloudflare_cache( get_permalink( $comment_object->comment_post_ID ) );
		}
	}
    ...
    public function clear_cloudflare_cache_on_publish( $post_id ) {
        ...
        $this->clear_cloudflare_cache( get_permalink( $post_id ) );
        $this->clear_cloudflare_cache( get_home_url() );
    }
    ...
    public function clear_cloudflare_cache( $url_to_purge = null ) {
	    ...
	    if ( $url_to_purge ) {
	        $body = wp_json_encode( [ 'files' => [ $url_to_purge ] ], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
	    } else {
	        $body = wp_json_encode( [ 'purge_everything' => true ], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
	    }
	    ...
	    file_put_contents( WP_CONTENT_DIR . '/cloudflare-api.log', $log_entry, FILE_APPEND );
	    ...
	    if ( is_wp_error( $response ) ) {
	        ...
	        if ( $response_code !== 200 ) {
	            error_log( 'Cloudflare cache purge failed with response code: ' . $response_code );
	        }
     ...
     */
    public function add_cache_control_to_redirects( $redirect_by, $status, $location ) {
        if ( $status === 301 ) {
    ...
    public function check_leaked_credentials() {
        if ( isset( $_SERVER[ 'HTTP_EXPOSED_CREDENTIAL_CHECK' ] ) ) {
            ...
            wp_logout();
            wp_redirect( wp_lostpassword_url() . '&reason=leaked_credentials' );
            exit;
        }
    }
    ...
    public function password_reset_notice( $message ) {
        if ( isset( $_GET[ 'reason' ] ) && $_GET[ 'reason' ] === 'leaked_credentials' ) {
			// translators: %1$s is have i been pwned.
            $message = '<p class="message" style="border-left-color:red;">' . sprintf( __( 'According to %1$s your login credentials have been exposed in a data breach. Please reset your password to secure your account.', 'pp-cf-utils' ), '<a href="https://haveibeenpwned.com/">have i been pwned</a> &amp; <a href="https://developers.cloudflare.com/waf/detections/leaked-credentials/">Cloudflare</a>' ) . '</p>' . $message;
        }
        return $message;
    }
    ...
    public function add_toolbar_button( $wp_admin_bar ) {
	    ...
	    $wp_admin_bar->add_node([
	        ...
	        'href'  => '#',
	        'meta'  => [
	            'title' => __( 'Cloudflare', 'pp-cf-utils' ), // Tooltip text
	        ],
	    ]);
        ...
        if ( ! $this->get_cloudflare_zone_id() ) {
            $wp_admin_bar->add_node( [
                ...
                'title' => __( 'Set Cloudflare zone settings', 'pp-cf-utils' ),
                'parent' => 'cloudflare',
        ...
        $wp_admin_bar->add_node( [
            ...
            'title' => __( 'Clear full CF cache', 'pp-cf-utils' ),
            'parent' => 'cloudflare',
			'href' => '#',
            'meta'  => [
		...
		if ( is_singular() ) {
	        $wp_admin_bar->add_node( [
	            ...
	            'title' => __( 'Clear CF cache for this URL', 'pp-cf-utils' ),
	            'parent' => 'cloudflare',
				'href' => '#',
	            'meta'  => [
	...
	public function toolbar_icon_styles() {
	    ...
	    wp_add_inline_style(
	        'admin-bar', // Adds to the admin bar's stylesheet
	        '#wp-admin-bar-cloudflare .ab-item .cf-icon { width: 15px; height: 20px; padding: 4px; display: inline-block; background-image: url(' .plugin_dir_url( __FILE__ ). 'cloudflare_icon.svg) !important; background-size: contain; }'
	    );
	}
    ...
    public function add_toolbar_button_script() {
        $nonce = wp_create_nonce( 'clear_cloudflare_cache_nonce' );
        ?>
        <script type="text/javascript">
            function clearCloudflareCache( url = '' ) {
				let confirm_msg = '<?php _e( 'Are you sure you want to clear the Cloudflare cache?', 'pp-cf-utils' ); ?>';
				if ( url !== '' ) {
					confirm_msg = '<?php _e( 'Are you sure you want to clear the Cloudflare cache for %s ?', 'pp-cf-utils' ); ?>'.replace( '%s', url );
				}
    ...
    public function ajax_clear_cloudflare_cache() {
		...
		if ( ! empty( $_POST['url'] ) ) {
			$this->clear_cloudflare_cache( $_POST['url'] );
		} else {
	        $this->clear_cloudflare_cache();
		}
        wp_send_json_success( [ 'message' => __( 'Cloudflare cache cleared successfully.', 'pp-cf-utils' ) ] );
    }
    ...
    public function register_settings() {
        register_setting( 'pp_cf_utils_settings_group', 'pp_cf_utils_settings', [
            'sanitize_callback' => [ $this, 'sanitize_settings' ]
        ]);

        add_settings_section(
            'pp_cf_utils_main_section',
            __( 'Zone settings', 'pp-cf-utils' ),
            null,
            'pp-cloudflare-utils'
        );
    ...
    public function sanitize_settings($input) {
        $output = [];
        $output['zone-id']   = sanitize_text_field( $input['zone-id'] );
        $output['email']     = sanitize_email( $input['email'] );
        $output['api-token'] = sanitize_text_field( $input['api-token'] );
        return $output;
    }
    ...
    private function render_settings_field( $field, $type = 'text' ) {
        ...
        $value         = defined( $constant_name ) ? constant( $constant_name ) : ( isset( $settings[ $field ] ) ? esc_attr( $settings[ $field ] ) : '' );
        $disabled      = defined( $constant_name ) ? 'disabled' : '';

        echo '<input type="' . esc_attr( $type ) . '" name="pp_cf_utils_settings[' . esc_attr( $field ) . ']" value="' . $value . '" class="regular-text" ' . $disabled . ' />';
    }
